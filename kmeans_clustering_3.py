...
# ------------------
embeddings = np.load("data/scraped_articles_9-15-politics_embeddings.npy").astype(
    "float32"
)
print(f"✅ Loaded embeddings: {embeddings.shape}")

# Load the JSON file that matches the embeddings
try:
    with open("data/scraped_articles_9-15-politics.json", "r", encoding="utf-8") as f:
        articles_data = json.load(f)

    # Convert to DataFrame
    df = pd.DataFrame(articles_data)

    # Add article_id if it doesn't exist (using index + 1)
    if "article_id" not in df.columns:
        df["article_id"] = range(1, len(df) + 1)

    print("✅ Loaded articles from JSON")
    print(f"📊 Loaded {len(df)} articles")
except FileNotFoundError:
    print("❌ Could not find scraped_articles_9-15-politics.json file")
    exit(1)
...
# ------------------
def find_optimal_k(embeddings, max_k=50, min_k=2):
    """
    ...
    k_range = range(min_k, min(max_k + 1, len(embeddings) // 2))
    inertias = []
    silhouette_scores = []
    ...
    for k in k_range:
        ...
        # Calculate silhouette score (only for reasonable cluster sizes)
    ...
    print(
        f"📊 Optimal k based on silhouette score: {optimal_k} (score: {max_silhouette:.3f})"
    )
...
optimal_k, inertias, silhouette_scores, k_range = find_optimal_k(embeddings, max_k=30)
...
# Save JSON
# ------------------
with open("data/kmeans_clusters.json", "w", encoding="utf-8") as f:
    json.dump(clusters_out, f, indent=2, ensure_ascii=False)

print("✅ Saved data/kmeans_clusters.json with titles and site names")
...
df_clusters.to_csv("data/kmeans_df_clusters.csv", index=False)

# Also save as JSON for compatibility
with open("data/kmeans_df_clusters.json", "w", encoding="utf-8") as f:
    json.dump(df_clusters.to_dict(orient="records"), f, indent=2, ensure_ascii=False)

print(
    "✅ Saved data/kmeans_df_clusters.csv and data/kmeans_df_clusters.json sorted by num_sites, then num_articles"
)
print(
...
# 4. Articles per region in clusters
region_counts = df_clusters["regions"].explode().value_counts()
axes[1, 1].bar(range(len(region_counts)), region_counts.values)
axes[1, 1].set_xlabel("Region")
axes[1, 1].set_ylabel("Number of Articles")
axes[1, 1].set_title("Articles per Region in Clusters")
axes[1, 1].set_xticks(range(len(region_counts)))
axes[1, 1].set_xticklabels(region_counts.index, rotation=45)
axes[1, 1].grid(True)

plt.tight_layout()
plt.savefig("data/kmeans_clustering_analysis.png", dpi=300, bbox_inches="tight")
print("✅ Saved clustering visualization to data/kmeans_clustering_analysis.png")
...
print(
    f"   - Files saved: kmeans_clusters.json, kmeans_df_clusters.csv, kmeans_df_clusters.json"
)
print(f"   - Visualization: kmeans_clustering_analysis.png")
